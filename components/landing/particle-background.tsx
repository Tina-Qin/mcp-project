...
export function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    // Initialize particles - increased count for denser network
    ...
    for (let i = 0; i < particleCount; i++) {
      particlesRef.current.push({
        ...
        isBlue: Math.random() > 0.4, // 60% blue, 40% white
      })
    }
    ...
    const animate = () => {
      ...
      particles.forEach((particle, i) => {
        particles.slice(i + 1).forEach((other) => {
          ...
          if (distance < 150) {
            ...
            if (particle.isBlue && other.isBlue) {
              gradient.addColorStop(0, `rgba(59, 130, 246, ${opacity})`)
              gradient.addColorStop(1, `rgba(37, 99, 235, ${opacity})`)
            } else if (particle.isBlue || other.isBlue) {
              gradient.addColorStop(0, `rgba(59, 130, 246, ${opacity})`)
      ...
      particles.forEach((particle) => {
        ...
        const color = particle.isBlue 
          ? `rgba(59, 130, 246, ${particle.opacity})`
          : `rgba(255, 255, 255, ${particle.opacity * 0.9})`
        
        // Outer glow
        ctx.beginPath()
        ctx.arc(particle.x, particle.y, particle.size * 2, 0, Math.PI * 2)
        ctx.fillStyle = particle.isBlue 
          ? `rgba(59, 130, 246, ${particle.opacity * 0.15})`
          : `rgba(255, 255, 255, ${particle.opacity * 0.1})`
        ctx.fill()
